...
import org.apache.hadoop.kudu.filter.BinaryComparator;
import org.apache.hadoop.kudu.filter.ByteArrayComparable;
import org.apache.hadoop.kudu.filter.CompareFilter.CompareOp;
import org.apache.hadoop.kudu.filter.Filter;
import org.apache.hadoop.kudu.filter.NullComparator;
import org.apache.hadoop.kudu.filter.PrefixFilter;
import org.apache.hadoop.kudu.filter.RegexStringComparator;
import org.apache.hadoop.kudu.filter.RowFilter;
import org.apache.hadoop.kudu.filter.SingleColumnValueFilter;

import com.google.common.base.Charsets;
import com.google.common.collect.ImmutableList;

public class KuduFilterBuilder extends AbstractExprVisitor<KuduScanSpec, Void, RuntimeException> implements DrillKuduConstants {
  ...
  public KuduScanSpec visitFunctionCall(FunctionCall call, Void value) throws RuntimeException {
    ...
    if (CompareFunctionsProcessor.isCompareFunction(functionName)) {
      /*
       * HBASE-10848: Bug in Kudu versions (0.94.[0-18], 0.96.[0-2], 0.98.[0-1])
       * causes a filter with NullComparator to fail. Enable only if specified in
       * the configuration (after ensuring that the Kudu cluster has the fix).
       */
      if (nullComparatorSupported == null) {
        nullComparatorSupported = groupScan.getKuduConf().getBoolean("drill.kudu.supports.null.comparator", false);
      }
  ...
  private KuduScanSpec mergeScanSpecs(String functionName, KuduScanSpec leftScanSpec, KuduScanSpec rightScanSpec) {
    ...
    switch (functionName) {
    case "booleanAnd":
      newFilter = KuduUtils.andFilterAtIndex(leftScanSpec.filter, KuduUtils.LAST_FILTER, rightScanSpec.filter);
      startRow = KuduUtils.maxOfStartRows(leftScanSpec.startRow, rightScanSpec.startRow);
      stopRow = KuduUtils.minOfStopRows(leftScanSpec.stopRow, rightScanSpec.stopRow);
      break;
    case "booleanOr":
      newFilter = KuduUtils.orFilterAtIndex(leftScanSpec.filter, KuduUtils.LAST_FILTER, rightScanSpec.filter);
      startRow = KuduUtils.minOfStartRows(leftScanSpec.startRow, rightScanSpec.startRow);
      stopRow = KuduUtils.maxOfStopRows(leftScanSpec.stopRow, rightScanSpec.stopRow);
    }
    return new KuduScanSpec(groupScan.getTableName(), startRow, stopRow, newFilter);
  }
  ...
  private KuduScanSpec createKuduScanSpec(FunctionCall call, CompareFunctionsProcessor processor) {
    ...
    if (!(isRowKey
      ...
      /*
       * if the field in this function is neither the row_key nor a qualified Kudu column, return.
       */
      return null;
    }
    ...
    case "like":
       ...
       */
      KuduRegexParser parser = new KuduRegexParser(call).parse();
      compareOp = CompareOp.EQUAL;
    ...
    if (compareOp != null || startRow != HConstants.EMPTY_START_ROW || stopRow != HConstants.EMPTY_END_ROW) {
      ...
      if (isRowKey) {
        ...
        byte[] family = KuduUtils.getBytes(field.getRootSegment().getPath());
        byte[] qualifier = KuduUtils.getBytes(field.getRootSegment().getChild().getNameSegment().getPath());
      ...
      return new KuduScanSpec(groupScan.getTableName(), startRow, stopRow, filter);
    }
    // else
    return null;
  }

private KuduScanSpec createRowKeyPrefixScanSpec(FunctionCall call,
